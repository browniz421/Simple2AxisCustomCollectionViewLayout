...
- (void)prepareLayout
{
  ...
  for (NSInteger i = 0; i < [self.collectionView numberOfItemsInSection:0]; i++) {
    ...
    origin.y = row * [self screenSize].height/ 10;
    NSInteger column = i%10;
    origin.x = column * [self screenSize].width/ 10;
    ...
    la.frame = CGRectMake(origin.x, origin.y, [self screenSize].width/ 10, [self screenSize].height / 10);
...
- (CGSize)collectionViewContentSize
{
  ...
  contentSize.width = screenSize.width / 10;
  contentSize.height = screenSize.height / 10;
  return contentSize;
}
